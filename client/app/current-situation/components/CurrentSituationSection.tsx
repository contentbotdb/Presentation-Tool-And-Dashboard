...
export const MAX_STEP_INDEX = STEPS.length - 1;

export type CurrentSituationSectionProps = {
  stepIndex: number;
  onPrev: () => void;
  onNext: () => void;
};

export function CurrentSituationSection({ stepIndex, onPrev, onNext }: CurrentSituationSectionProps) {
      ...
      <div className="min-w-0 flex-1 overflow-hidden rounded-lg bg-white shadow-md">
        ...
        <div className="border-t border-zinc-100 p-4">
          <div className="flex items-center justify-between gap-4">
            <h2 className="text-2xl font-semibold text-black">{STEPS[stepIndex].label}</h2>
          ...
          <p className="mt-2 text-[#525252]">{STEP_DESCRIPTIONS[selectedId]}</p>
        </div>
      </div>
